...
public class WorkPlaceController extends BaseController {
	...
	@ResponseBody
	public Object selectWorkPlaceList(Integer size, Integer current, Integer state) {
		if (size == null) {
			size = 7;
		}
		if (current == null) {
		...
		if (state == null) {
			...
			Wrapper<Tb_work_place> wrapper = new EntityWrapper<Tb_work_place>();
			wrapper.where("w_state=1");
			Page<Tb_work_place> page = new Page<>(current,size);
			Page<Tb_work_place> pageListPage = iTb_work_placeService.selectPage(page,wrapper);
